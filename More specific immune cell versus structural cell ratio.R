### 2021-05-27
# Project SCS 46 patients - More specific macrophage and T cell versus structural cell ratio
# Jonne Damen
...
## Cluster 3 is resident-like macrophages
seurdata@meta.data$new.ident[seurdata@meta.data$new.ident == "3"] <- "resident like macrophages"


## Cluster 6 is inflammatory macrophages
seurdata@meta.data$new.ident[seurdata@meta.data$new.ident == "6"] <- "inflammatory macrophages"


## Cluster 10 is foam cells
seurdata@meta.data$new.ident[seurdata@meta.data$new.ident == "10"] <- "foam cells"


## Cluster 0 and 7 are CD8+ T cells
tcells <- c("0", "7")
# Rename clusters
for(cluster in tcells){
  seurdata@meta.data$new.ident[seurdata@meta.data$new.ident == cluster] <- "CD8+ T cells"
}

## Cluster 1 and 5 are CD4+ T cells
tcells <- c("1", "5")
# Rename clusters
for(cluster in tcells){
  seurdata@meta.data$new.ident[seurdata@meta.data$new.ident == cluster] <- "CD4+ T cells"
}
...
cell.options <- c("resident like macrophages", "inflammatory macrophages", "foam cells", "CD4+ T cells", "CD8+ T cells")
...
all.ratios <- lapply(cell.options, function(cl){
  ...
  # Make factors of clusters
  df$cluster <- as.factor(df$cluster)
  
  # Set order of factors
  df$cluster <- ordered(df$cluster, levels = celltypes)
